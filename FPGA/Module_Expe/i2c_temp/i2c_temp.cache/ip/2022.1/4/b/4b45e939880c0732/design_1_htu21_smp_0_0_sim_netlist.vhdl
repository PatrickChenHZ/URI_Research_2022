...
architecture STRUCTURE of decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix_htu21_smp is
  ...
  signal reg_len_i_1_n_0 : STD_LOGIC;
  signal \^rw\ : STD_LOGIC;
  ...
  attribute SOFT_HLUTNM of reg_len_i_1 : label is "soft_lutpair1";
begin
  iic_en <= \^iic_en\;
  iic_rst <= \^iic_rst\;
...
reg_len_i_1: unisim.vcomponents.LUT4
    generic map(
      INIT => X"FD04"
    )
        port map (
...
reg_len_reg: unisim.vcomponents.FDRE
     port map (
      C => clk,
      CE => '1',
      D => reg_len_i_1_n_0,
...
architecture STRUCTURE of decalper_eb_ot_sdeen_pot_pi_dehcac_xnilix is
  ...
  signal \^iic_write\ : STD_LOGIC_VECTOR ( 5 to 5 );
  signal \^led\ : STD_LOGIC_VECTOR ( 1 downto 0 );
...
begin
  iic_write(7) <= \<const0>\;
  iic_write(6) <= \<const0>\;
  iic_write(5) <= \^iic_write\(5);
  iic_write(4) <= \^iic_write\(5);
  iic_write(3) <= \<const0>\;
  iic_write(2) <= \^iic_write\(5);
  iic_write(1) <= \<const0>\;
  iic_write(0) <= \<const0>\;
  led(3) <= \<const0>\;
  led(2) <= \<const0>\;
