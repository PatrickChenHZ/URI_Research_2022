...
SoftwareSerial SerialPynq(pynqRX,pynqTX);
...
void setup() {
  ...
  //Serial2.setRxBufferSize(6*packetSize);     // the default buffer size of 64 is too small
  //SerialPynq.begin(115200,SWSERIAL_8N1,pynqRX,pynqTX,false,128,0);    // try getting the input char by char and see if readstruntil is actually the issue
  SerialPynq.begin(115200);
  //SerialSens.begin(115200,SWSERIAL_8N1,sensRX,sensTX,false,128,0);

  Serial2.setTimeout(40);
  SerialPynq.setTimeout(40);
  //SerialSens.setTimeout(40);

  //xTaskCreatePinnedToCore(RecvSensCode, "receiving sens packets", 10000, NULL, 3, &RecvSensHandle, 0);
  xTaskCreatePinnedToCore(RecvPynqCode, "receiving pynq packets", 10000, NULL, 5, &RecvPynqHandle, 0);
  xTaskCreatePinnedToCore(RecvArduCode, "receiving ardu packets", 10000, NULL, 2, &RecvArduHandle, 0);
  xTaskCreatePinnedToCore(SendCode, "sending packets", 100000, NULL, 1, &SendHandle, 1);
